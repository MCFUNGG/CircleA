...
try {
    ...
    // Update match status to 'P' (Pending)
    $update_query = "UPDATE `match` SET status = 'P' WHERE match_id = ?";
    ...
    $notification_message = $is_tutor ? "導師 {$sender_name} 已接受您的配對請求" : "學生 {$sender_name} 已接受您的配對請求";
    ...
    if ($token_result && $token_row = mysqli_fetch_assoc($token_result)) {
        ...
        // 準備 FCM 消息，使用與 send_fcm_test.php 相同的格式
        // 讀取服務帳戶憑證
    ...
    error_log("Error in update_match_status_to_Pending.php: " . $e->getMessage());
