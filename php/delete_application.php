...
try {
    ...
    // 0. 更新 tutor_rating 表中的相關記錄，將 application_id 設為 NULL，而不是刪除
    $update_ratings = "UPDATE tutor_rating SET application_id = NULL WHERE application_id = ?";
    error_log("Update ratings query: $update_ratings with app_id=$app_id");
    
    $stmt = $conn->prepare($update_ratings);
    if (!$stmt) {
        throw new Exception("準備更新評分語句失敗: " . $conn->error);
    }
    
    $stmt->bind_param('i', $app_id);
    $stmt->execute();
    error_log("Ratings updated: " . $stmt->affected_rows);
